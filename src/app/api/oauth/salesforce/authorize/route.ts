...
export async function GET(request: NextRequest) {
  ...
  let clientId: string;
  try {
    const cfg = await convex.action((api as any).private.salesforce.getOAuthAppConfig, {
      entityId,
    });
    oauthBaseUrl = String(cfg?.oauthBaseUrl ?? "").replace(/\/$/, "");
    clientId = String(cfg?.clientId ?? "");
  } catch (e) {
    const message = e && typeof e === "object" && "message" in e ? String((e as any).message) : "Missing Salesforce OAuth app config";
    return NextResponse.redirect(
      new URL(
  ...
  if (!clientId || !oauthBaseUrl) {
    return NextResponse.redirect(
      new URL(
        `/integrations?error=${encodeURIComponent("Missing Salesforce OAuth app config")}`,
        baseUrl,
      ),
    );
  }
  ...
  const authUrl = new URL(`${oauthBaseUrl}/services/oauth2/authorize`);
  ...
  response.cookies.set("salesforce_oauth_ctx", ctx, {
    httpOnly: true,
    secure: isHttps,
    sameSite: isHttps ? "none" : "lax",
    maxAge: 600,
