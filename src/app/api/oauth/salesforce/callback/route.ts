...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    const msg = details ? `Salesforce OAuth error: ${error} (${details})` : `Salesforce OAuth error: ${error}`;
    return NextResponse.redirect(
      new URL(
        `/integrations?error=${encodeURIComponent(msg)}`,
        baseUrl,
      ),
    );
  }
  ...
  const stateCookie = request.cookies.get("salesforce_oauth_state")?.value;
  if (!stateCookie || stateCookie !== state) {
    ...
    response.cookies.delete("salesforce_oauth_ctx");
    return response;
  }

  const ctxCookie = request.cookies.get("salesforce_oauth_ctx")?.value;
  if (!ctxCookie) {
    const response = NextResponse.redirect(
      new URL(
        `/integrations?error=${encodeURIComponent("Missing OAuth context. Please retry connecting Salesforce.")}`,
        baseUrl,
      ),
    );
    response.cookies.delete("salesforce_oauth_state");
  ...
  let stateData: SalesforceState;
  try {
    stateData = JSON.parse(Buffer.from(ctxCookie, "base64url").toString("utf-8"));
  } catch {
    const response = NextResponse.redirect(
      new URL(
        `/integrations?error=${encodeURIComponent("Invalid state data")}`,
        baseUrl,
      ),
    );
    response.cookies.delete("salesforce_oauth_state");
  ...
  try {
    await convex.action((api as any).private.salesforce.completeOAuthConnection, {
    ...
    const message =
      e && typeof e === "object" && "message" in e
        ...
        : "Failed to complete Salesforce OAuth";
    const response = NextResponse.redirect(
      new URL(
