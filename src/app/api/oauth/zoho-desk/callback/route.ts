...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    const msg = details
      ? `Zoho Desk OAuth error: ${error} (${details})`
      : `Zoho Desk OAuth error: ${error}`;
    return NextResponse.redirect(
  ...
  const stateCookie = request.cookies.get("zoho_desk_oauth_state")?.value;
  if (!stateCookie || stateCookie !== state) {
    ...
    response.cookies.delete("zoho_desk_oauth_state");
    response.cookies.delete("zoho_desk_oauth_ctx");
    return response;
  }

  const ctxCookie = request.cookies.get("zoho_desk_oauth_ctx")?.value;
  if (!ctxCookie) {
    const response = NextResponse.redirect(
      new URL(
        `/integrations?error=${encodeURIComponent("Missing OAuth context. Please retry connecting Zoho Desk.")}`,
        baseUrl,
      ),
    );
    response.cookies.delete("zoho_desk_oauth_state");
  ...
  try {
    await convex.action((api as any).private.zohoDesk.completeOAuthConnection, {
    ...
    const message =
      e && typeof e === "object" && "message" in e
        ...
        : "Failed to complete Zoho Desk OAuth";
    const response = NextResponse.redirect(
