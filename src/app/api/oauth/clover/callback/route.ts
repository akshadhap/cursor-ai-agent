...
export async function GET(request: NextRequest) {
  ...
  const merchantId = searchParams.get("merchant_id") ?? searchParams.get("merchantId");
  const error = searchParams.get("error");

  if (error) {
    const msg = `Clover OAuth error: ${error}`;
    return NextResponse.redirect(
  ...
  if (!code || !state || !merchantId) {
    return NextResponse.redirect(
      new URL(
        `/integrations?error=${encodeURIComponent("Missing authorization code, merchant_id, or state")}`,
        baseUrl,
      ),
    );
  }

  const stateCookie = request.cookies.get("clover_oauth_state")?.value;
  if (!stateCookie || stateCookie !== state) {
    ...
    response.cookies.delete("clover_oauth_ctx");
    return response;
  }

  const ctxCookie = request.cookies.get("clover_oauth_ctx")?.value;
  if (!ctxCookie) {
    const response = NextResponse.redirect(
      new URL(
        `/integrations?error=${encodeURIComponent("Missing OAuth context. Please retry connecting Clover.")}`,
        baseUrl,
      ),
    );
    response.cookies.delete("clover_oauth_state");
  ...
  try {
    await convex.action((api as any).private.clover.completeOAuthConnection, {
    ...
    const message =
      e && typeof e === "object" && "message" in e
        ...
        : "Failed to complete Clover OAuth";
    const response = NextResponse.redirect(
