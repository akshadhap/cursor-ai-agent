...
export async function GET(request: NextRequest) {
    try {
        ...
        const credential = await prisma.credential.create({
            data: {
                name: `Google Calendar (OAuth - ${new Date().toLocaleDateString()})`,
        ...
        return NextResponse.redirect(
            new URL(
                `/credentials/${credential.id}?success=Google Calendar connected successfully!`,
                appUrl
            )
        );
    } catch (error) {
        ...
        return NextResponse.redirect(
            new URL(
                `/credentials/new?error=${encodeURIComponent(`Google Calendar OAuth failed: ${errorMessage}`)}`,
                process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"
            )
        );
    }
