...
export async function GET(request: NextRequest) {
    try {
        ...
        const credential = await prisma.credential.create({
            data: {
                name: `Google Drive (OAuth - ${new Date().toLocaleDateString()})`,
        ...
        return NextResponse.redirect(
            new URL(
                `/credentials/${credential.id}?success=Google Drive connected successfully!`,
                appUrl
            )
        );
    } catch (error) {
        ...
        return NextResponse.redirect(
            new URL(
                `/credentials/new?error=${encodeURIComponent("Failed to complete OAuth flow. Please try again.")}`,
                process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"
            )
        );
    }
