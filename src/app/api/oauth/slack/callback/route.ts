...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    const msg = details
      ? `Slack OAuth error: ${error} (${details})`
  ...
  const stateCookie = request.cookies.get("slack_oauth_state")?.value;
  if (!stateCookie || stateCookie !== state) {
    ...
    response.cookies.delete("slack_oauth_ctx");
    return response;
  }

  const ctxCookie = request.cookies.get("slack_oauth_ctx")?.value;
  if (!ctxCookie) {
    const response = NextResponse.redirect(
      new URL(
        `/integrations?error=${encodeURIComponent(
          "Missing OAuth context. Please retry connecting Slack.",
        )}`,
        baseUrl,
      ),
    );
  ...
  if (stateData.entityId !== currentOrgId) {
    const response = NextResponse.redirect(
      new URL(
        `/integrations?error=${encodeURIComponent(
          "Organization mismatch during OAuth flow",
        )}`,
        baseUrl,
      ),
    );
  ...
  try {
    await convex.action((api as any).private.slack.completeOAuthConnection, {
  ...
  const response = NextResponse.redirect(
    new URL(`${safeReturnTo}?connected=true`, baseUrl),
  );
  response.cookies.delete("slack_oauth_state");
  response.cookies.delete("slack_oauth_ctx");
