...
export async function GET(request: NextRequest) {
  ...
  let scopes: string;
  try {
    const cfg = await convex.action((api as any).private.hubspot.getOAuthAppConfig, {
      entityId,
    });
    clientId = String(cfg?.clientId ?? "");
    scopes = String(cfg?.scopes ?? "oauth").trim();
  } catch (e) {
    const message =
      e && typeof e === "object" && "message" in e
        ? String((e as any).message)
        : "Missing HubSpot OAuth app config";
    return NextResponse.redirect(
  ...
  if (!clientId) {
    return NextResponse.redirect(
      new URL(
        `/integrations?error=${encodeURIComponent("Missing HubSpot OAuth app config")}`,
        baseUrl,
      ),
    );
  }
  ...
  response.cookies.set("hubspot_oauth_ctx", ctx, {
    httpOnly: true,
    secure: isHttps,
    sameSite: isHttps ? "none" : "lax",
    maxAge: 600,
